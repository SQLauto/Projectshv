...
BULK INSERT #PercentGrowth FROM 'F:\Monitor\PercentGrowth\Query_PercentGrowth_Reports.txt' WITH ( FIELDTERMINATOR = '|', ROWTERMINATOR = '\n')
...
if (select count(*) from #PercentGrowth) <>0
...
EXEC msdb.dbo.sp_send_dbmail @recipients=@mail_address ,
    @subject = 'PROD - Database Status Alert (Percent Auto Growth)',
    @body = @tableHTML,
