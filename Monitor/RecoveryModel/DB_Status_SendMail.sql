...
BULK INSERT #RecoveryModel FROM 'F:\Monitor\RecoveryModel\RecoveryModel_Reports.txt' WITH ( FIELDTERMINATOR = '|', ROWTERMINATOR = '\n')
...
if (select count(*) from #RecoveryModel) <>0
...
EXEC msdb.dbo.sp_send_dbmail @recipients=@mail_address ,
    @subject = 'PROD - Database Status Alert (RECOVERY MODEL)',
    @body = @tableHTML,
