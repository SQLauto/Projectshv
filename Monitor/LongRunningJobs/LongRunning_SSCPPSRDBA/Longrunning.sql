...
--IF EXISTS(select * from tempdb..sysobjects where name like '%#LongJobs%')
--BEGIN
--DROP TABLE #LongJobs
--END
...
, 'Threshold is set to: '  +  CAST( isnull(dbExc.IntParam1, isnull(srvExc.IntParam1,120)) as varchar(50)) + ' minutes' AS [Max Threshold Minutes] 
...
and DATEDIFF(minute, [start_execution_date], GETDATE()) > isnull(dbExc.IntParam1, isnull(srvExc.IntParam1,120))  
