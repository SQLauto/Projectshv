...
BULK INSERT #LongJobs FROM 'F:\Monitor\LongRunningJobs\Longrunning_Report.txt' WITH ( FIELDTERMINATOR = '|', ROWTERMINATOR = '\n')
...
BULK INSERT #OperatorFromFile FROM 'F:\Monitor\LongRunningJobs\Longrunning_Operators.txt' WITH ( FIELDTERMINATOR = '|', ROWTERMINATOR = '\n')
...
select * from #OperatorFromFile
 ...
 DECLARE @VAR_ADDRESS nVARCHAR(MAX);
   ...
   -- No counting of rows
	...
	SELECT a.email_address + '; oscar_espinoza@ryder.com; Christian_Tafur@ryder.com; Sarvapriya_Tripathi@ryder.com; salvador_sandoval@ryder.com; Natarajan_Palani@ryder.com'
	FROM #Operator a
...
EXEC msdb.dbo.sp_send_dbmail 
     ...
     @subject = 'SSCVPETESQL03 - Long Running Jobs',
     @body = @tableHTML,
     @body_format = 'HTML',
...
SET @VAR_ADDRESS = 'Ryder_DBA@ryder.com; Sarvapriya_Tripathi@ryder.com; Natarajan_Palani@ryder.com'
EXEC msdb.dbo.sp_send_dbmail 
     @recipients = @VAR_ADDRESS,
     @subject = 'SSCVPETESQL03 - Long Running Jobs',
     @body = @tableHTML,
