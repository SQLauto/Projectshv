...
BULK INSERT #LongJobs FROM 'F:\Monitor\LongRunningJobs\LongRunning_AMDATA\Longrunning_Report.txt' WITH ( FIELDTERMINATOR = '|', ROWTERMINATOR = '\n')
...
BULK INSERT #OperatorFromFile FROM 'F:\Monitor\LongRunningJobs\LongRunning_AMDATA\Longrunning_Operators.txt' WITH ( FIELDTERMINATOR = '|', ROWTERMINATOR = '\n')
...
select * from #OperatorFromFile
 ...
 DECLARE @VAR_ADDRESS nVARCHAR(MAX);
   ...
   -- No counting of rows
    ...
    SELECT distinct a.email_address + '; oscar_espinoza@ryder.com; Adrian_Robles@ryder.com'
	FROM #Operator a
...
EXEC msdb.dbo.sp_send_dbmail 
     ...
     @subject = 'AMDATA - Long Running Jobs',
     @body = @tableHTML,
     @body_format = 'HTML',
