...
BULK INSERT #DBstatus FROM 'F:\Monitor\DB_STATUS\DBstatus_Reports.txt' WITH ( FIELDTERMINATOR = '|', ROWTERMINATOR = '\n')
...
if (select count(*) from #DBstatus) <>0
...
EXEC msdb.dbo.sp_send_dbmail @recipients=@mail_address ,
    @subject = 'PROD - Database Status Alert (DB STATUS)',
    @body = @tableHTML,
