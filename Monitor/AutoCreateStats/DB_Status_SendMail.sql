...
BULK INSERT #AutoCreateStats FROM 'F:\Monitor\AutoCreateStats\AutoCreateStats_Reports.txt' WITH ( FIELDTERMINATOR = '|', ROWTERMINATOR = '\n')
...
if (select count(*) from #AutoCreateStats) <>0
...
EXEC msdb.dbo.sp_send_dbmail @recipients=@mail_address ,
    @subject = 'PROD - Database Status Alert (AUTO CREATE STATS)',
    @body = @tableHTML,
