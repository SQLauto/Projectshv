...
BULK INSERT #AutoClose FROM 'F:\Monitor\AutoClose\AutoClose_Reports.txt' WITH ( FIELDTERMINATOR = '|', ROWTERMINATOR = '\n')
...
if (select count(*) from #AutoClose) <>0
...
EXEC msdb.dbo.sp_send_dbmail @recipients=@mail_address ,
    @subject = 'PROD - Database Status Alert (AUTO CLOSE)',
    @body = @tableHTML,
