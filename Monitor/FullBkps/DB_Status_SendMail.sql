...
BULK INSERT #fullbkps FROM 'F:\Monitor\FullBkps\Query_FullBkps_Reports.txt' WITH ( FIELDTERMINATOR = '|', ROWTERMINATOR = '\n')
...
if (select count(*) from #fullbkps) <>0
...
EXEC msdb.dbo.sp_send_dbmail @recipients=@mail_address ,
    @subject = 'PROD - Database Status Alert (Full Backups)',
    @body = @tableHTML,
