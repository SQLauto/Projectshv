...
BULK INSERT [#MaxAGReport] FROM 'F:\Monitor\MaxAutoGrowth\Query_MaxAG_Reports.txt' WITH ( FIELDTERMINATOR = '|', ROWTERMINATOR = '\n')
...
if (select count(*) from #MaxAGReport) <>0
...
EXEC msdb.dbo.sp_send_dbmail @recipients=@mail_address ,
    @subject = 'PROD - Database Status Alert (Max Auto Growth)',
    @body = @tableHTML,
    @body_format = 'HTML',@profile_name ='Ryder DBA'; 
END
ELSE
BEGIN
DROP TABLE [#MaxAGReport]                   
END