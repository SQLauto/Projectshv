...
BULK INSERT #LongHoldLocks FROM 'F:\Monitor\HoldLocks\Query_HoldLocks_Reports.txt' WITH ( FIELDTERMINATOR = '|', ROWTERMINATOR = '\n')
...
if (select count(*) from #LongHoldLocks) <>0
...
<div class="testNormal"> Please check out the Database status below</div><br>
  <table class="center" border=1>
    <tr>
       <th>InstanceName</th> 
       <th>Dbname</th>      
       <th>Blocker_session_id</th>
	<th>Blocker_login_name</th>
	<th>Blocker_host_name</th>
	<th>Blocker_program_name</th>
	<th>Blocked_session_id</th>
	<th>Blocked_wait_time_seconds</th>
	<th>Blocked_login_name</th>
	<th>Blocked_host_name</th>
	<th>Blocked_program_name</th>
	<th>Blocked_wait_resource</th>
	<th>MinutesMax</th>
	<th>CheckWarning</th>
      </tr>'+CAST ( ( SELECT td = Instance_Name,       '',
     td = DBName,       '',
      td = Blocker_session_id, '',
	  td = Blocker_login_name, '',
	  td = Blocker_host_name, '',
	  td = Blocker_program_name, '',
	  td = Blocked_session_id, '',
	  td = Blocked_wait_time_seconds, '',
	  td = Blocked_login_name, '',
	  td = Blocked_host_name, '',
	  td = Blocked_program_name, '',
      td = Blocked_wait_resource, '',
	  td = MinutesMax, '',
	  td = CheckWarning, ''
                     FROM #LongHoldLocks                   
...
EXEC msdb.dbo.sp_send_dbmail @recipients=@mail_address ,
    @subject = 'PROD - Database Status Alert (Long Hold Locks)',
    @body = @tableHTML,
