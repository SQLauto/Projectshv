...
go
CREATE TABLE [dbo].[#FilesFull]
(Instance_Name varchar(256),
 DataFile varchar(256),
 PhysicalName varchar (512),
 Mbtotal varchar(128),
 MBUsed varchar(128),
 PercetangeUsed varchar(128),
 ExpectedPercentage varchar(256),
  CheckWarning varchar(256))

BULK INSERT #FilesFull FROM 'F:\Monitor\FilesFull\Query_FilesFull_Reports.txt' WITH ( FIELDTERMINATOR = '|', ROWTERMINATOR = '\n')
...
if (select count(*) from #FilesFull) <>0
...
<div class="testNormal"> Please check out the Database status below</div><br>
  <table class="center" border=1>
    <tr>
	   ...
	   <th>Mbtotal</th>
	   <th>MBUsed</th>
	   <th>PercetangeUsed</th>
	   <th>ExpectedPercentage</th>
	   <th>CheckWarning</th>
...
EXEC msdb.dbo.sp_send_dbmail @recipients=@mail_address ,
    @subject = 'PROD - Database Status Alert (Data File Getting Full)',
    @body = @tableHTML,
    @body_format = 'HTML',@profile_name ='Ryder DBA'; 
END
ELSE
BEGIN
DROP TABLE #FilesFull                   
END